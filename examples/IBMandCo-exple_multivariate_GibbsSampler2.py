...
if DEBUG:   
    print np.linalg.inv(V_priormean)
    print np.linalg.inv(W2_priormean)
sample_size = 100000
AllThetas = np.zeros((T+1, 2*nbstock, sample_size))
AllinvV = np.zeros((nbstock, nbstock, sample_size))
...
for ii in range(sample_size):
    ...
    # Sample from (V, W2 | theta_{0:T}, Y_{1:T})
    Fth = np.zeros(Y.shape)
    Gb = np.zeros((timesteps, nbstock))
    for jj, (F, Gfull, th, b_im) in enumerate(zip(Ft, Gt, thetas, betas_iminusone)):
        Fth[jj,:] = (F.dot(th.reshape((2*nbstock,1)))).T
        G = Gfull[nbstock:,nbstock:]
        Gb[jj,:] = (G.dot(b_im.reshape((nbstock,1)))).T
    YmFth = (Y-Fth).T
    if DEBUG:
        print "\n", invS0
        print YmFth.dot(YmFth.T)
    invV = Wishart_fromSigmaInverse(nu0+T, invS0 + YmFth.dot(YmFth.T), 1e-11)
    bmGbi = (betas_i - Gb).T
...
if SAVEFILE == True:    
    ...
    '-mu0' + str(int(mu0)) + \
    '-nu0' + str(int(nu0))
    np.savez_compressed(namefile, AllThetas, AllinvW2, AllinvV)
